...
{
    public float moveUpDistance = 1.5f; // 올라가는 거리
    public float moveSpeed = 3f; // 올라가는 속도
    public float waitTime = 2f; // 올라간 후 대기 시간
    ...
    IEnumerator MoveSaw()
    {
        ...
        {
            // 위로 올라가기
            yield return MoveToPosition(transform.position + new Vector3(0, moveUpDistance, 0), moveSpeed);
            // 대기
